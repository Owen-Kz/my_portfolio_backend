...
const retrieveFiles = async (req, res) => {
  try {
    ...
    let countQuery = `SELECT COUNT(DISTINCT pi.item_id) AS total FROM portfolio_items pi`;
    const queryParams = [];

    // Add category filter if specified and not 'All'
    if (category && category !== 'All') {
      itemsQuery += ` WHERE c.name = ?`;
      countQuery += ` JOIN categories c ON pi.category_id = c.category_id WHERE c.name = ?`;
